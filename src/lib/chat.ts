...
const HEURISTIC_FALLBACKS: { match: RegExp; reply: string }[] = [
  ...
  {
    match: /^(yes|yeah|yep|sure|ok|okay|alright|sounds good|that works|let'?s do it|cool)\s*[.!?]*\s*$/i,
    reply:
  ...
  {
    match: /\b(bye|goodbye|see ?ya|talk later|cya|farewell|catch you later|see you (later|soon))\b/i,
    reply:
  ...
  {
    match: /\b(awesome|amazing|excellent|wonderful|fantastic|love (it|this)|impressive|brilliant|incredible)\b/i,
    reply:
  ...
  {
    match: /\b(disappointed|bad service|complaint|complain|terrible|awful|frustrated|unhappy|not (happy|good|great)|angry|upset)\b/i,
    reply:
  ...
  {
    match: /\bhow (are|r)( you| u| things)?\b|\bhow('?s| is) (it going|life|things|everything|your day)\b|\bhow do you do\b|\bhow have you been\b|\bwhat'?s up\b|\bwhatsup\b|\bsup\b/i,
    reply:
  ...
  // ===== Identity =====
  {
    match: /\b(what|who)\s+(is|are|does)\s+(j\.?\s*o\.?\s*a\.?\s*t\.?|joat)( kenya)?\b|\babout joat\b|tell me about joat|founded|since 1983|\bhistory\b/i,
    reply:
  ...
  {
    match: /\bwho are you\b|\bwhat are you\b|\bare you (a|an) (bot|ai|robot|human|person)\b|\byour name\b/i,
    reply:
  ...
  {
    match: /^(hi|hello|hey|howdy|greetings|hola|good\s+(morning|afternoon|evening|day))\s*[!.?]*\s*$/i,
    reply:
  ...
  // ===== Bot capability / help =====
  {
    match: /^(help|menu|options)\s*[.!?]*\s*$|what can (you|u) (do|help)|what (can|do) you help|things you can|how can you assist/i,
    reply:
  ...
  {
    match: /\b(ecommerce|e-commerce|online store|shopify|woocommerce|magento|cart|checkout|payment gateway|stripe integration|online shop|marketplace site)\b/i,
    reply:
  ...
  // ===== Tech stack =====
  {
    match: /\btech\s?stack\b|technolog(y|ies)|frameworks?|\b(react|nextjs|next\.js|vue|svelte|flutter|kotlin|swift|nodejs|node\.js|python|django|rails|laravel|supabase|cloudflare|aws|gcp|azure|firebase|postgres|mongodb|typescript|tailwind|deno)\b/i,
    reply:
  ...
  {
    match: /software|mobile app|web app|\bweb(site|app)?\b|develop|engineer(ing)?|backend|frontend|api\b|saas|\bplatform\b|\bcms\b|dashboard|admin panel/i,
    reply:
  ...
  {
    match: /\b(google ads|meta ads|facebook ads|instagram ads|tiktok ads|linkedin ads|youtube ads|search ads|display ads|ppc|paid search)\b/i,
    reply:
  ...
  {
    match: /marketing|campaign|social media|\bads?\b|\bbrand\b|\bseo\b|paid media|growth marketing|content strategy/i,
    reply:
  ...
  {
    match: /media|animation|\bvideo\b|blender|\b3d\b|motion|explainer|content production|brand film|videograph|cinematograph/i,
    reply:
  ...
  {
    match: /\bai\b|claude|openrouter|automation|\bllm\b|\bagent\b|chatbot for|generative|\bgpt\b|gemini|machine learning|\bml\b/i,
    reply:
  ...
  // ===== Process / engagement =====
  {
    match: /\b(process|how do you (work|engage|operate)|how it works|workflow|methodology|approach|discovery|kickoff|stages|phases)\b/i,
    reply:
  ...
  {
    match: /\b(how long|how many (days|weeks|months)|timeline|turnaround|delivery time|how (fast|quick|soon)|when (can|will|would) (i|we|you)|deadline|\beta\b|completion time)\b/i,
    reply:
  ...
  {
    match: /\b(start (a |my )?project|begin (a |my )?project|how do i (begin|start|engage)|get started|first step|kick.?off|onboard|sign me up|engagement process)\b/i,
    reply:
  ...
  {
    match: /\b(portfolio|examples|case stud(y|ies)|previous work|past project|past client|reference|samples?|showcase)\b|who have you worked with|work you have done/i,
    reply:
  ...
  {
    match: /\b(testimonial|review|client (story|stories|feedback)|what do (clients|customers) say|happy clients)\b/i,
    reply:
  ...
  // ===== Packages / pricing =====
  {
    match: /package|tier|\bplan\b|pricing|price|cost|quote|budget|foundation|\bgrowth\b|\bscale\b|how much|\$|estimate/i,
    reply:
  ...
  {
    match: /\b(pay(ment|ing)?|invoice|billing|m-?pesa|paypal|stripe|wire|swift|currency|\busd\b|\bkes\b|kenyan shilling|deposit|payment terms|how do i pay)\b/i,
    reply:
  ...
  {
    match: /\b(maintenance|support|sla|response time|bug fix|hotfix|update my (app|site|website))\b/i,
    reply:
  ...
  // ===== Reach / scope =====
  {
    match: /\b(industry|industries|sector|vertical|fintech|edtech|health.?tech|health ?care|retail|ecommerce|e-commerce|logistics|nonprofit|ngo|government|saas|b2b|b2c|media company|news company)\b/i,
    reply:
  ...
  {
    match: /\b(international|global|remote|outside (kenya|africa)|abroad|\busa?\b|\buk\b|europe|asia|middle east|north america|canada)\b|work(ing)? (with|for) clients (in|across|worldwide)/i,
    reply:
  ...
  {
    match: /\b(nda|non-?disclosure|confidential(ity)?|privacy of (project|idea)|secret project|sign an? nda)\b/i,
    reply:
  ...
  {
    match: /\b(data privacy|gdpr|data protection|where is my data|data residency|host(ing|ed) (in|on))\b/i,
    reply:
  ...
  {
    match: /meeting|\bbook\b|\bdemo\b|schedule|appointment|consult|calendly|talk to|speak to|set.?up.+call|jump on a call/i,
    reply:
  ...
  {
    match: /\b(office hours|working hours|open|closed|business hours|when (are|do) you (open|available|working)|time zone|timezone)\b/i,
    reply:
  ...
  {
    match: /\bcontact\b|\bemail\b|\bphone\b|\bcall\b|reach (out|us)|get in touch|how do i (reach|contact)/i,
    reply:
  ...
  {
    match: /\b(facebook|fb|instagram|\big\b|insta|linkedin|youtube|yt\b|tiktok|twitter|\bx\.com\b|social (media|channels?|accounts?)|follow (you|us))\b/i,
    reply:
  ...
  {
    match: /\bcareer|\bapply\b|\bintern|join your team|work for you|work at joat|open role|open position|\bjobs? (at|with|in) joat|cv|resume|portfolio submission/i,
    reply:
  ...
  {
    match: /\bhire\b|\bhiring\b|recruit(er|ing|ment)?|outsourc|staffing|head ?hunt|find (a |me |us )?(talent|candidate|developer|engineer|hire)|need (a |to )?(talent|developer|engineer|hire|candidates?)|workforce|source (talent|candidates)|place (a |the )?candidate/i,
    reply:
